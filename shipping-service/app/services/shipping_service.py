...
from app.kafka.shipping_producer import send_event  # kafka  thing
...
VALID_TRANSITIONS = {
    "CREATED": [{"type": "STATUS", "value": "SHIPPED"}],
    ...
    "RETURN_REQUESTED": [{"type": "STATUS", "value": "RETURNED"}],
    "REPLACEMENT_REQUESTED": [{"type": "STATUS", "value": "REPLACED"}],
    "RETURNED": [],
    "REPLACED": [],
}
...
def can_transition(current: str, new: str) -> bool:
    ...
    allowed_values = [
        action["value"] for action in actions if action.get("type") == "STATUS"
    ]
...
def create_shipment_service(order_id: int, db):
    ...
    try:
        order_res = requests.get(f"{ORDER_SERVICE_URL}/orders/{order_id}", timeout=3)
...
def update_shipment_status_service(
    shipment_id: int, new_status: str, db, location: Optional[str] = None
):
...
def add_tracking_update_service(
    shipment_id: int, data, db, location: Optional[str] = None
):
...
def get_next_actions_service(shipment_id: int, db):
    ...
    return {"current": current, "allowed_actions": actions}
